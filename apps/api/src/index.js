...
app.post('/api/format-leak-profile', optionalAuth, userRateLimit(10, 15 * 60 * 1000), async (req, res) => {
  try {
    ...
    const prompt = `Проанализируй предоставленные данные об утечках для номера телефона или email и представь полный отчет в формате профессионального аудита кибербезопасности. Отчет должен содержать следующие блоки:
...
2. **Карта цифрового следа:**
   Создай текстовую ASCII-схему, которая наглядно показывает, как различные данные пользователя (телефон, ФИО, адрес) связаны между собой через конкретные утечки. Используй стрелки и указания источников.
...
app.post('/api/ai-leak-analysis', optionalAuth, userRateLimit(5, 15 * 60 * 1000), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const userPrompt = `Проанализируй предоставленные данные об утечках для запроса "${query}" (поле: ${field}) и представь полный отчет в формате профессионального аудита кибербезопасности. Отчет должен содержать следующие блоки:
