...
app.post('/api/ai-leak-analysis', optionalAuth, userRateLimit(5, 15 * 60 * 1000), async (req, res) => {
  ...
  try {
    ...
    try {
      // Системный промпт для анализа утечек безопасности (оптимизированный)
      const systemPrompt = `Ты эксперт кибербезопасности. Анализируй данные утечек быстро и структурированно.
...
Отвечай сжато, на русском, с эмодзи.`;
...
Создай профессиональный анализ безопасности с практическими рекомендациями.`;
      ...
      const completion = await openai.chat.completions.create({
        ...
        max_tokens: 2000, // Уменьшил для более быстрого ответа
        temperature: 0.2, // Более низкая температура для скорости
