    ...
    <div className={cn("relative min-h-screen overflow-hidden bg-slate-50 text-slate-900", ptMono.className)}>
      ...
      <header className="border-b border-slate-200 bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
          <Link href="/" className="text-xs font-semibold uppercase tracking-[0.5em] text-emerald-700 hover:text-emerald-500">
            DataTrace
          </Link>
        ...
        <section className="relative border-b border-slate-200">
          <div className="mx-auto grid max-w-6xl gap-8 px-6 py-16 lg:grid-cols-[3fr_2fr] lg:items-end">
            <div className="space-y-6">
              <span className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white px-4 py-1 text-xs uppercase tracking-[0.4em] text-emerald-600">
            ...
            <div className="space-y-5 rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-[0_20px_60px_rgba(45,212,191,0.2)]">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-600/90">
                Что внутри подборки
              </p>
              ...
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-xs uppercase tracking-[0.3em] text-emerald-600">
          ...
          <div className="mx-auto grid max-w-6xl gap-10 px-6 py-16 lg:grid-cols-[320px_1fr] xl:grid-cols-[360px_1fr]">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {post.tags.slice(0, 2).map((tag) => (
                          <Badge
                            key={tag}
                            variant="outline"
                            className="border-emerald-200 bg-emerald-50 text-[10px] uppercase tracking-[0.35em] text-emerald-600"
                          >
                            {tag}
                          </Badge>
                        ))}
            ...
            <article className="space-y-8 rounded-3xl border border-slate-200 bg-white/80 p-8 shadow-[0_30px_80px_rgba(45,212,191,0.25)]">
              <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.35em] text-slate-500">
                ...
                <span className="inline-flex items-center gap-2">
                  <Clock className="h-4 w-4 text-emerald-500" /> {activePost.readingTime}
                </span>
                <Link
                ...
                <div className="flex flex-wrap gap-2">
                  {activePost.tags.map((tag) => (
                    <Badge
                      key={tag}
                      variant="outline"
                      className="border-emerald-200 bg-emerald-50 text-[11px] uppercase tracking-[0.35em] text-emerald-600"
                    >
                      {tag}
                    </Badge>
                  ))}
              ...
              <ul className="space-y-3 rounded-2xl border border-slate-200 bg-white/80 p-6 text-sm text-slate-700">
              ...
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-600/80">Следите за обновлениями</p>
                  ...
                  <Button
                    asChild
                    className="h-12 rounded-full border border-emerald-300 bg-white px-6 text-xs font-semibold uppercase tracking-[0.35em] text-emerald-600 hover:bg-emerald-100"
                  >
