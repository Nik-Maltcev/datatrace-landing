...
export function BlogLanding({ posts }: BlogLandingProps) {
  ...
  return (
    <div className={cn("relative min-h-screen overflow-hidden bg-slate-950 text-slate-100", ptMono.className)}>
      ...
      <header className="border-b border-white/10 bg-black/10 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
          <Link href="/" className="text-xs font-semibold uppercase tracking-[0.5em] text-emerald-200/80 hover:text-emerald-100">
            DataTrace
          </Link>
          <div className="flex items-center gap-4 text-xs uppercase tracking-[0.35em] text-slate-400">
            <Link href="/" className="hover:text-emerald-200">
              Главная
            </Link>
            <span className="inline-flex items-center gap-1 rounded-full border border-emerald-400/60 bg-emerald-400/10 px-3 py-1 text-emerald-100">
        ...
        <section className="relative border-b border-white/10">
          <div className="mx-auto grid max-w-6xl gap-8 px-6 py-16 lg:grid-cols-[3fr_2fr] lg:items-end">
            <div className="space-y-6">
              <span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/40 bg-emerald-400/10 px-4 py-1 text-xs uppercase tracking-[0.4em] text-emerald-200">
            ...
            <div className="space-y-5 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_20px_60px_rgba(16,185,129,0.15)]">
              ...
              <div className="rounded-2xl border border-emerald-400/40 bg-emerald-400/10 p-4 text-xs uppercase tracking-[0.3em] text-emerald-200">
          ...
          <div className="mx-auto grid max-w-6xl gap-10 px-6 py-16 lg:grid-cols-[320px_1fr] xl:grid-cols-[360px_1fr]">
            <aside className="space-y-5">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">Последние заметки</p>
                ...
                {posts.map((post) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "group w-full rounded-2xl border border-white/10 bg-white/5 p-5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-400/60 hover:bg-white/10",
                        isActive && "border-emerald-400/70 bg-emerald-400/10 shadow-[0_20px_40px_rgba(16,185,129,0.25)]"
                      )}
                    >
                      ...
                      <h3 className="mt-4 text-base font-semibold text-white">{post.title}</h3>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {post.tags.slice(0, 2).map((tag) => (
                          <Badge
                            key={tag}
                            variant="outline"
                            className="border-emerald-400/30 bg-emerald-400/10 text-[10px] uppercase tracking-[0.35em] text-emerald-200"
                          >
                            {tag}
                          </Badge>
                        ))}
            ...
            <article className="space-y-8 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-[0_30px_80px_rgba(15,118,110,0.2)]">
              <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.35em] text-slate-400">
                <span className="inline-flex items-center gap-2">
                  <CalendarDays className="h-4 w-4 text-emerald-200" /> {formatDate(activePost.date)}
                </span>
                <span className="inline-flex items-center gap-2">
                  <Clock className="h-4 w-4 text-emerald-200" /> {activePost.readingTime}
                </span>
                <Link
              ...
              <div className="space-y-6">
                <h2 className="text-3xl font-semibold text-white lg:text-4xl">{activePost.title}</h2>
                <p className="text-base text-slate-300 lg:text-lg">{activePost.summary}</p>
                <div className="flex flex-wrap gap-2">
                  {activePost.tags.map((tag) => (
                    <Badge
                      key={tag}
                      variant="outline"
                      className="border-emerald-400/40 bg-emerald-400/10 text-[11px] uppercase tracking-[0.35em] text-emerald-200"
                    >
                      {tag}
                    </Badge>
                  ))}
              ...
              <ul className="space-y-3 rounded-2xl border border-white/10 bg-black/20 p-6 text-sm text-slate-200">
              ...
              <div className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 p-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-200/80">Следите за обновлениями</p>
                  ...
                  <Button
                    asChild
                    className="h-12 rounded-full border border-emerald-400/60 bg-white/10 px-6 text-xs font-semibold uppercase tracking-[0.35em] text-emerald-100 hover:bg-emerald-400/20"
                  >
